...
<html lang="es-ES" dir="ltr" xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script language="JavaScript">
        ...
        function editar(id_){
            window.open("mn_usuario21.php?id_persona="+id_,"_self");
        }
        function usuariosis(id_){
            window.open("mn_usuario23.php?id_persona="+id_,"_self");
        }
        function menu(id_){
            window.open("mn_usuario24.php?id_persona="+id_,"_self");
        }
    </script>
...
if(isset($_POST['chkusuarios'])){$chkusuarios=$_POST['chkusuarios'];}
...
<form name='form1' method="post" action="mn_usuario2.php">
    ...
    <span class="form-el"><input type='checkbox' name='chkusuarios' checked="tru">Solo usuarios del Sistema</span>
...
if(!empty($condicion)){
    ...
    if($chkusuarios=='on'){
        $consulta="SELECT id_persona,tipo_iden,identificacion,CONCAT(pnombre,' ',snombre,' ',papellido,' ',sapellido) AS nombre,direccion,telefono FROM vw_usuario_sist WHERE ".$condicion." ORDER BY ".$orden;
    }
    else{
    ...
    if($consulta->num_rows<>0){
        ...
        while($row=$consulta->fetch_array()){
            ...
            echo "<td width='5%'><a href='#' onclick=usuariosis($row[id_persona],$row[identificacion]) title='Información de Usuarios del Sistema' class='btnhref'><span class='icon-user'></span></a></td>";
            echo "<td width='5%'><a href='#' onclick=menu($row[id_persona],$row[identificacion]) title='Opciones del Menú' class='btnhref'><span class='icon-menu'></span></a></td>";  
