...
<html lang="es-ES" dir="ltr" xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script language="JavaScript">
        function validar(){
            document.form1.action='mn_inf_saldo1.php';
            document.form1.target='_self';
            document.form1.submit();
        }
        function imprimir(){
            //document.form1.action='mn_inf_saldo11.php';
            document.form1.action='mn_inf_saldo12.php';
...
if(isset($_POST['chk_saldocero'])){
    $chk_saldocero=$_POST['chk_saldocero'];
    $checked='checked';
}
?>
<div class="caja1">
    <h4>Informe de Saldos de Medicamentos y Dispositivos</h4>
</div>
<form name='form1' method="post" action="mn_inf_recepcion1.php">
    ...
    <span class="form-el"><input type='checkbox' id='chk_saldocero' name='chk_saldocero' <?php echo $checked?>/>Mostrar Saldos en Cero</span>
    <a href="#" onclick='validar();' title='Buscar'><span class="icon-magnifying-glass"></span> </a>
<?php
$orden='id_ingreso,id_producto';
$condicion="";
if($chk_saldocero<>'on'){$condicion=$condicion."saldo<>0 AND ";}
...
if(!empty($condicion)){
    ...
    $consulta="SELECT id_ingreso, fecha_ing, tipo_iden,identificacion,CONCAT(papellido,' ',sapellido) AS apellidos ,CONCAT(pnombre,' ',snombre) AS nombres, id_producto, CONCAT(descripcion,' ',concentracion,' ',presentacion) AS producto ,cantidad_ingresa, cantidad_aplicada,saldo FROM vw_inventario_paciente WHERE ".$condicion." ORDER BY ".$orden;
    //echo "<br>".$consulta;
    $sql_=$consulta;
    ...
    if($consulta->num_rows<>0){
        ...
        while($row=$consulta->fetch_array()){
            if($ingreso!=$row['id_ingreso']){
                echo "<tr><td><b>Nro. Ingreso: $row[id_ingreso]</td></tr>";
            ...
            //echo "<tr>";
            //echo "<td></td>";
            echo "<td align='right'>$row[cantidad_ingresa]</td>";
            echo "<td align='right'>$row[cantidad_aplicada]</td>";
