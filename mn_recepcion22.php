...
if($link->affected_rows > 0){
    ...
    $consultainv="SELECT inventario_paciente.id_inventario, inventario_paciente.cantidad_ingresa FROM inventario_paciente WHERE inventario_paciente.id_ingreso='$id_ingreso' AND id_producto='$id_producto'";
    //echo "<br>".$consultainv;
    $link->query($consultainv);
    $consultainv=$link->query($consultainv);
    $rowinv=$consultainv->fetch_array();
    $sql_="UPDATE inventario_paciente SET cantidad_ingresa=$rowinv[cantidad_ingresa]-$cantidad WHERE id_inventario='$rowinv[id_inventario]'";
    //echo "<br>".$sql_;
    $link->query($sql_);
}
